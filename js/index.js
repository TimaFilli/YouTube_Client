...
async function renderUsers(users, truncate) {
    ...
    for (let user of users) {
        usersList.innerHTML += `
            ...
            (function(){
                window.localStorage.setItem('lastUserId', ${user.userId})
                renderVideos(CACHED_VIDEOS.filter(v => v.user.userId == ${user.userId}), true)
            })()
            ">
                <a href="#">
                    <img src="${API + '/' + user.avatar}" alt="channel-icon" width="30px" height="30px">
...
async function renderVideos(videos, truncate) {
    ...
    for (let video of videos) {
        ...
        li.dataset.id = video.videoId

        li.innerHTML = `
            <video src="${API + '/' + video.link}" controls=""></video>
            <div class="iframe-footer">
                <img src="${API + '/' + video.user.avatar}" alt="channel-icon">
